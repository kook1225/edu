...
@interface AddReceiveAddressViewController () <UIPickerViewDelegate,UIPickerViewDataSource,UITextFieldDelegate,UIAlertViewDelegate> {
    UIView *sessionView;
    int addFlag;
                        ...
                        if ([_ZipCodeTextField.text length] == 0) {
                            ...
                            [manager POST:urlStr parameters:parameter
                                  success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                      ...
                                      if ([responseObject[@"responseCode"] intValue] == 0) {
                                          UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"新增成功" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                                          alert.tag = 201;
                                          [alert show];
                                      }
                                      else {
